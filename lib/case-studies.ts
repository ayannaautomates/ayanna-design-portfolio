...
export const letsAskSpiritCaseStudy: CaseStudy = {
  slug: "lets-ask-spirit",
  title: "Let's Ask Spirit",
  subtitle:
    "A medium's Wix site rebuilt by hand for people who arrive after a death: audited to WCAG 2.1 AA, cut from 11.2 MB to 4.8 MB, and shipped with a strict content security policy and not one tracking cookie.",
  meta: [
    { label: "Client", value: "Michael Edsel, Let's Ask Spirit" },
    { label: "Sector", value: "Spiritual Practice / Solo Practitioner" },
    { label: "Team Size", value: "1" },
    { label: "Location", value: "Rural Virginia" },
    {
      label: "Core Features",
      value: "Scroll-Driven Doorway, WCAG 2.1 AA, Four-Part Legal Suite",
    },
    { label: "Status", value: "Live at letsaskspirit.com" },
  ],
  stats: [
    {
      value: "57%",
      label: "lighter homepage after the scroll sequence was rebuilt",
    },
    {
      value: "9",
      label: "accessibility barriers found and fixed before launch",
    },
    {
      value: "0",
      label: "trackers, analytics scripts, or advertising cookies",
    },
  ],
  phases: [
    {
      ...
      title: "Executive Summary",
      paragraphs: [
        "Michael Edsel is an evidential psychic medium working out of a mini farm in rural Virginia. Most people who find his website have recently lost someone. That single fact set every decision in the rebuild.",
        "The result is a seven-page hand-coded site. The homepage opens on a wooden doorway that swings wide as you scroll, drawn frame by frame on a canvas element with no framework and no third-party JavaScript. Behind that sits the work nobody photographs: a full WCAG 2.1 AA audit with every Level A and AA barrier closed, a homepage payload cut by more than half, a content security policy strict enough to run without unsafe-inline, and a site that sets no tracking cookies and loads no analytics at all.",
      ],
    },
    {
      id: "problem",
      ...
      title: "The Problem",
      paragraphs: [
        "The previous site was a Wix template. It worked, in the sense that it existed. What it could not do was carry the weight of what people come to this practice for, or answer the question every prospective client has and almost none of them ask out loud: what does evidential actually mean, and how is this different from a guess?",
        "There was a second, quieter problem. A practice like this attracts scepticism, and it serves people at their most vulnerable. A site that feels careless, or that quietly ships someone's grief to an analytics dashboard, undermines the work before a word is read.",
      ],
      bullets: [
        "A rented template on a platform that decided what the homepage could do",
        "No answer anywhere to the one question that separates this practice from a cold reading",
        "No policy pages, on a service where refunds, boundaries, and scope genuinely matter",
        "Nothing in place for visitors using a screen reader, a keyboard, or reduced motion",
      ],
    },
    {
      id: "approach",
      ...
      title: "The Approach",
      paragraphs: [
        "The visible half was a doorway. It is the practice's own metaphor, so the homepage makes you open it: a photographic sequence painted onto a canvas and scrubbed against scroll position, with the client's copy revealed underneath.",
        "The invisible half mattered more. Every finding was measured rather than assumed, and several assumptions turned out to be wrong. Converting the animation frames to WebP at their original size made the files larger, not smaller, because the source JPEGs were already well compressed. Frame count and resolution were the real levers. That is the whole method here: change one thing, measure it, keep it only if the number moves.",
      ],
      bullets: [
        "Client-supplied legal copy shaped into four pages: terms, refunds, disclaimer, privacy",
        "A privacy policy written from what the code actually does, not from a generator's assumptions",
        "Colour tokens split so the brand gold passes contrast on both light and dark backgrounds",
        "Reduced motion honoured throughout, including the doorway, which renders already open",
      ],
    },
    {
      id: "architecture",
      ...
      title: "Architecture",
      paragraphs: [
        "Seven static pages of HTML, one stylesheet, one JavaScript file. No build step, no framework, no package manager, no database. Bookings run through Calendly and both forms post to FormSubmit, so there is no server to maintain and no stored personal data to leak.",
        "Hosting moved from a Wix template to Netlify, deploying from a private repository on every push. The domain stayed registered at Wix, where the DNS records were repointed by hand, since Wix does not allow nameserver changes from its own DNS screen. Response headers, clean URLs, and cache lifetimes are all declared in configuration rather than bolted on.",
      ],
      bullets: [
        "Content security policy allowlisting only the origins the site genuinely uses",
        "No unsafe-inline, which meant moving the last inline style attributes into the stylesheet first",
        "HSTS, nosniff, Referrer-Policy, X-Frame-Options and Permissions-Policy set at the host",
        "Sitemap, robots, canonical tags, Open Graph, and LocalBusiness schema with real prices",
      ],
    },
    {
      id: "results",
      ...
      title: "Results",
      paragraphs: [
        "The homepage went from 11.2 MB to 4.8 MB. The doorway sequence, which was 97 JPEGs requested all at once, is now 49 WebP frames that stream in after the first one has already painted, so the hero is visible before the animation finishes downloading.",
        "A full WCAG 2.1 AA audit turned up ten findings and no critical barriers. Nine were fixed before launch: a carousel that advanced itself with no way to stop it, no skip link, no main landmark, a homepage with no h1, heading levels that skipped, an unlabelled decorative canvas, and a focus ring that was invisible against the dark sections. The tenth was a verification note, not a defect.",
      ],
      bullets: [
        "Every image carries alt text and every form control is properly labelled",
        "Contrast passes AA sitewide, including the gold, which is used only on dark backgrounds",
        "The certificate, the redirects, and all seven pages verified on the live domain, not locally",
        "A launch social kit and a captions document handed over with the site",
      ],
    },
    {
      id: "why-it-matters",
      ...
      title: "Why This Matters",
      paragraphs: [
        "Accessibility, performance, and privacy get treated as enterprise concerns, the sort of thing you get to once you are big enough. That is backwards. A solo practitioner has less margin for a visitor who cannot use the site, not more, and the people arriving here are often distracted, upset, and on a phone with poor signal.",
        "None of this work is visible in a screenshot. A visitor will never know the carousel can be paused, that the doorway holds still if their system asks for reduced motion, or that nothing followed them off the page. They will only notice that the site loaded quickly and felt calm. That is the point.",
      ],
    },
  ],
};
