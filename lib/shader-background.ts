export const cyberpunkShaderSource = `#version 300 es
/*********
 * Based on Matthias Hurrle (@atzedent) — retinted for cyberpunk palette
 */
...
void main(void) {
  ...
  for (float i=1.; i<12.; i++) {
    ...
    vec2 p=uv;
    float d=length(p);
    col+=.00125/d*(cos(sin(i)*vec3(0.15,2.4,3.0)+vec3(0.0,0.55,1.0))+1.);
    float b=noise(i+p+bg*1.731);
    col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
    col=mix(col, vec3(bg*0.06, bg*0.18, bg*0.22), d);
  }
  O=vec4(col, 1.0);
}`;
...
void main(){gl_Position=position;}`;
  ...
  updatePointerCoords(coords: number[]) {
    this.pointerCoords = [coords[0], coords[1]];
  }
  ...
  setup() {
    ...
    this.compile(this.fs, cyberpunkShaderSource);
